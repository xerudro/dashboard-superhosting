...
const CurrencyServices = () => {
  ...
  const canManageCurrency = hasPermission('manage_currency');

  const handleUpdateRate = async (fromCurrency: string, toCurrency: string) => {
    if (!canManageCurrency) {
      toast.error('You do not have permission to update currency rates');
      return;
    }
  ...
  const handleAddNewRate = async () => {
    if (!canManageCurrency) {
      toast.error('You do not have permission to add currency rates');
      return;
    }
  ...
  return (
    <div className="space-y-6">
                  ...
                  {editingRate === `${rate.from_currency}-${rate.to_currency}` ? (
                    <div className="flex items-center space-x-2">
                      ...
                      <button
                        ...
                        disabled={isLoading || !canManageCurrency}
      ...
      {/* Rate History */}
      <div className="bg-gray-900/50 backdrop-blur-sm rounded-xl p-6">
        <h2 className="text-xl font-semibold text-white mb-6">Rate History</h2>
            ...
            <tbody>
              {rateHistory.map((history) => (
                <tr key={history.id} className="border-b border-gray-800">
                  ...
                  <td className="py-4 text-gray-400">Admin</td>
                </tr>
              ))}
            </tbody>
          </table>
